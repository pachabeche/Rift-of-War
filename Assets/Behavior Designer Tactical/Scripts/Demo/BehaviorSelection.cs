...
namespace BehaviorDesigner.Runtime.Tactical
{
    public class BehaviorSelection : MonoBehaviour
    {
        ...
        public GameObject secondaryLeader;
        public GameObject agentGroup;
        public GameObject enemyGroup;
        public GameObject defendObject;
        public GUISkin descriptionGUISkin;

        private Dictionary<int, BehaviorTree> agentBehaviorTreeGroup = new Dictionary<int, BehaviorTree>();
        private BehaviorTree secondaryLeaderBehaviorTree;
        ...
        public void Start()
        {
            ...
            for (int i = 0; i < behaviorTrees.Length; ++i) {
                agentBehaviorTreeGroup.Add(behaviorTrees[i].Group, behaviorTrees[i]);
            }
            behaviorTrees = secondaryLeader.GetComponents<BehaviorTree>();
            for (int i = 0; i < behaviorTrees.Length; ++i) {
                if (behaviorTrees[i].Group != 0) {
                    secondaryLeaderBehaviorTree = behaviorTrees[i];
                }
            }

            var allAgents = agentGroup.GetComponentsInChildren<BehaviorTree>();
            for (int i = 0; i < allAgents.Length; ++i) {
                if (allAgents[i].gameObject != leader && !allAgents[i].Equals(secondaryLeaderBehaviorTree)) {
                    agents.Add(allAgents[i]);
                }
            }
            enemyHealth = enemyGroup.GetComponentsInChildren<Health>();
            behaviorTrees = enemyGroup.GetComponentsInChildren<BehaviorTree>();
            for (int i = 0; i < behaviorTrees.Length; ++i) {
        ...
        private void SelectionChanged()
        {
            ...
            agentBehaviorTreeGroup[(int)prevSelectionType].DisableBehavior();
            for (int i = 0; i < agents.Count; ++i) {
                agents[i].DisableBehavior();
            }
            if (secondaryLeaderBehaviorTree.Group == (int)prevSelectionType) {
                secondaryLeaderBehaviorTree.DisableBehavior();
            }
        ...
        private IEnumerator EnableBehavior()
        {
            ...
            agentBehaviorTreeGroup[(int)selectionType].EnableBehavior();
            for (int i = 0; i < agents.Count; ++i) {
                if (secondaryLeaderBehaviorTree.gameObject.Equals(agents[i].gameObject) && secondaryLeaderBehaviorTree.Group == (int)selectionType) {
                    secondaryLeaderBehaviorTree.EnableBehavior();
                } else {
